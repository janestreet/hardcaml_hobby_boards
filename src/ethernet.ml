    ...
    Always.(
      compile
        ...
        ; state.switch
            ...
            ; ( SFD
              , [ data_out_valid <-- vdd
                ; data_out <-- sel_top preamble_sfd_buffer.value ~width:output_data_width
                ; preamble_sfd_buffer
            ...
            ; ( Data
              , [ data_out_valid <-- vdd
                ...
                ; when_
                    (last_data_word
                     &: (uresize data_counter.value ~width:(width last_word_valid_cycles)
                         ==: last_word_valid_cycles))
                    [ fcs <-- ~:(fcs_next.value); state.set_next FCS ]
                ] )
            ; ( FCS
